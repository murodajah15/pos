<?php
	...
	if (isset($_POST['check1'])) {
		...
		$queryh = mysqli_query($connect,"select * from order_part where order_part.proses='Y'");
	}else{
		$tanggal = $tanggal1.' s/d '.$tanggal2;
		$queryh = mysqli_query($connect,"select * from order_part where order_part.proses='Y' and (tglorder>='$tgl1' and tglorder<='$tgl2')");
	}
	...
	if (isset($_POST['check1'])) {
		...
		$tanggal = $tanggal1.' s/d '.$tanggal2;
		$queryd = mysqli_query($connect,"select order_part.noorder,order_part.tglorder,order_part.nowo,order_part.nopolisi,order_partd.kdbarang,order_partd.nmbarang,order_partd.satuan,order_partd.qty,order_partd.harga,order_partd.discount,order_partd.subtotal from order_part inner join order_partd on order_part.noorder=order_partd.noorder where order_part.proses='Y' and (order_part.tglorder>='$tgl1' and order_part.tglorder<='$tgl2')");
	}
	...
	$html = '<style>
		...
		<font size="2"><br><p><center>LAPORAN ORDER PART BODY REPAIR</p>
		...
		<table table-layout="fixed"; cellpadding="0"; cellspacing="0"; style=font-size:11px; class="table table-striped table table-bordered;">
			<tr>
				...
				<th width="90px"><font size="1" color="black">NO. ORDER</th>
				<th width="60px"><font size="1" color="black">TGL. ORDER</th>
				...
				<th width="80px"><font size="1" color="black">HARGA</th>					
	...
	while ($row = mysqli_fetch_assoc($queryd))
	{
		$html .= '<tr>
				...
				<td width="90px" height="35px">&nbsp;'.$row["noorder"].'</td>
				<td width="70px" height="35px" align="center">'.date('d-m-Y', strtotime($row["tglorder"])).'</td>
				<td width="90px" height="35px">&nbsp;'.$row["nowo"].'</td>
				<td width="70px" height="35px">&nbsp;'.$row["nopolisi"].'</td>
				<td width="100px" height="35px">&nbsp;'.$row["kdbarang"].'</td>
				<td width="200px" height="35px">&nbsp;'.$row["nmbarang"].'</td>
				<td width="80px" height="35px" align="right">'.$row["qty"].'&nbsp;</td>
				<td width="80px" height="35px" align="right">'.$row["harga"].'&nbsp;</td>
				<td width="80px" height="35px" align="right">'.$row["subtotal"].'&nbsp;</td>
			</tr>';
		$no++;
	}

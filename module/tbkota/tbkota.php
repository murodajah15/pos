<?php
    ...
    if(isset($_GET['tipe'])){
        if($_GET['tipe']=='import'){
			cekakses($connect,$user,'Tabel Kota');
			$lakses = $_SESSION['aksestambah'];
			if ($lakses == 1) {
				echo "<font face='calibri'>
                <h3>Import Data Tabel Kota</h3>
                <form method='post' enctype='multipart/form-data' action='module/tbkota/proses_import.php'>
    ...
    if(isset($_GET['tipe'])){
        if($_GET['tipe']=='export'){
			...
			if ($lakses == 1) {
				echo "<font face='calibri'>
                <h3>Export Data Tabel Kota</h3>
                <form method='post' enctype='multipart/form-data' action='module/tbkota/proses_export.php'>
				<input type='hidden' name='username' value=$user>
    ...
    if(isset($_GET['tipe'])){
        if($_GET['tipe']=='detail'){
			...
			if ($lakses == 1) {	
				$sql=mysqli_query($connect,"select * from tbkota where id='$_GET[id]'");
				$de=mysqli_fetch_assoc($sql);?>
				<font face='calibri'>
					<h3>Detail Data Tabel Kota</h3>
    ...
    if(isset($_GET['tipe'])){
        if($_GET['tipe']=='tambah'){
			...
			if ($lakses == 1) {?>
				<font face='calibri'>
	                <h3>Tambah Data Tabel Kota</h3>
	                <form method='post' enctype='multipart/form-data' action='module/tbkota/proses_tambah.php'>
						<input type='hidden' name='username' value='<?= $user ?>'>
						<table style=font-size:13px; class='table table-striped table table-bordered'>
			...
			if ($lakses == 1) {	
				...
				$de=mysqli_fetch_assoc($sql);?>
				<font face='calibri'>
					<h3>Edit Data Tabel Kota</h3>
					<form method='post' enctype='multipart/form-data' action='module/tbkota/proses_edit.php'>
						<input type='hidden' name='username' value='<?= $user ?>'>
						<input type='hidden' name='id' value='<?= $de['id'] ?>'/>
						<table style=font-size:13px; class='table table-striped table table-bordered'>
						<tr><td>Kode</td> <td>  <input type=text class='form-control' name='kode' value='<?= $de['kode'] ?>' readonly></td></tr>
...
<div class="row">	 
	...
	<a class="btn btn-danger" href="?m=tbkota&tipe=tambah">Tambah data</a>
	<a class="btn btn-success" href="?m=tbkota&tipe=import">Import data</a>
	<a class="btn btn-warning" href="?m=tbkota&tipe=export">Export data</a>
</div>
</form>
<br>
...
<table class="table table-bordered table-striped table-hover">
	...
	// Cek apakah terdapat data page pada URL
		...
		if (empty($_POST['kata'])){
			$tampil = mysqli_query($connect,"SELECT * FROM tbkota order by kode LIMIT ".$limit_start.",".$limit);
			}
		else{
			$cari = $_POST['kata'];
			$tampil = mysqli_query($connect,"SELECT * FROM tbkota WHERE kode like '%$cari%' or nama like '%$cari%' order by kode LIMIT ".$limit_start.",".$limit);
		}			
        ...
        while($k=mysqli_fetch_assoc($tampil)){
            echo "<tr>
				...
				<td><u><a href='?m=tbkota&tipe=detail&id=$k[id]'><font color='blue'>$k[kode]</font></a></u></td>
				<td>$k[nama]</td>
				<td width='70' align='center'>$k[aktif]</td>
				<td>$k[user]</td>
                <td align='center' width='140px'>
                    <a class='btn btn-info' href='?m=tbkota&tipe=edit&id=$k[id]'>Edit</a>";
					cekakses($connect,$user,'Tabel Kota');
					$lakses = $_SESSION['akseshapus'];
					if ($lakses == 1) {
						/*echo " <a class='btn btn-danger' href='module/tbkota/proses_hapus.php?id=$k[id]&kode=$k[kode]'
...
<?php
	$query = mysqli_query($connect,"select count(*) as jumrec from tbkota");
...
?>
	...
	<ul class="pagination">
		...
		?>
		  <li><a href="dashboard.php?m=tbkota&page=1">First</a></li>
		  <li><a href="dashboard.php?m=tbkota&page=<?php echo $link_prev; ?>">&laquo;</a></li>
		<?php
		}
		?>
		...
		if (empty($_POST['kata'])){
			$tampil = mysqli_query($connect,"SELECT * FROM tbkota");
			}
		else{
			$cari = $_POST['kata'];
			$tampil = mysqli_query($connect,"SELECT * FROM tbkota WHERE kode like '%$cari%' or nama like '%$cari%'");
		}
		...
		?>
		  <li<?php echo $link_active; ?>><a href="dashboard.php?m=tbkota&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
		<?php
		}
		?>
		...
		?>
		  <li><a href="dashboard.php?m=tbkota&page=<?php echo $link_next; ?>">&raquo;</a></li>
		  <li><a href="dashboard.php?m=tbkota&page=<?php echo $jumlah_page; ?>">Last</a></li>
		<?php
		}
		?>
	</ul>
...
?>
 ...
 <script>
    function alert_hapus($id){
        ...
        .then((willDelete) => {
          if (willDelete) {
          	$href = "module/tbkota/proses_hapus.php?id=";

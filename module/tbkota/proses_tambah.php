<body>
	...
	<?php
		if(!empty($_POST['nama'])){
			...
			//Cek Double
			$cek = mysqli_num_rows(mysqli_query($connect,"select * from tbkota where kode='$_POST[kode]'"));
			if ($cek > 0){
				/*echo "<script>alert('Kode sudah ada, data tidak bisa disimpan !');history.go(-1) </script>";**/
				?>
				<script>
					swal({title: "Gagal simpan data ", text: "Kode tersebut sudah ada!", icon: 
			...
			// $query = mysqli_query($connect,"insert into tbkota (kode,nama,aktif,user) values 
			...
			$query = $connect->prepare("INSERT INTO tbkota (kode,nama,aktif,user) values (?,?,?,?)");

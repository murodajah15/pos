...
if (isset($_GET['tipe'])) {
	if ($_GET['tipe'] == 'import') {
		cekakses($connect, $user, 'Tabel Discount');
		$lakses = $_SESSION['aksestambah'];
		if ($lakses == 1) { ?>
			<font face='calibri'>
				<div class="panel panel-success">
					<div class="panel-heading">
						<font size="4">IMPORT DATA TABEL DISCOUNT</font>
					</div>
					<div class="panel-body">
						<form method='post' enctype='multipart/form-data' action='module/tbdiscount/proses_import.php'>
		...
		if ($lakses == 1) { ?>
			<font face='calibri'>
				<div class="panel panel-default">
					<div class="panel-heading">
						<font size="4">EXPORT DATA TABEL DISCOUNT</font>
					</div>
					<div class="panel-body">
						<form method='post' enctype='multipart/form-data' action='module/tbdiscount/proses_export.php'>
							<input type='hidden' name='username' value='<?= $user ?>'>
...
if (isset($_GET['tipe'])) {
	if ($_GET['tipe'] == 'detail') {
		...
		if ($lakses == 1) {
			// $query = $connect->prepare("select * from tbdiscount where id=?");
			...
			$sql = mysqli_query($connect, "select * from tbdiscount where id='$_GET[id]'");
		...
		?>
			<font face='calibri'>
				<div class="panel panel-default">
					<div class="panel-heading">
						<font size="4">DETAIL DATA TABEL DISCOUNT</font>
					</div>
					<div class="panel-body">
						<form method='post' enctype='multipart/form-data'>
							...
							<input button type='Button' class='btn btn-danger btn-sm' value='Close' onClick="window.location.href='?m=tbdiscount'" />
					</div>
				</div>
				</form>
			</font>
		...
		if ($lakses == 1) { ?>
			<font face='calibri'>
				<div class='panel panel-danger'>
					<div class='panel-heading'>
						<font size="4">TAMBAH DATA TABEL DISCOUNT</font>
					</div>
					<div class='panel-body'>
						<form method='post' name='tbdiscount' enctype='multipart/form-data' action='module/tbdiscount/proses_tambah.php'>
							<input type='hidden' name='username' value="<?= $user ?>">
		...
		?>
			<font face="calibri">
				<div class="panel panel-warning">
					<div class="panel-heading">
						<font size="4">EDIT DATA TABEL DISCOUNT</font>
					</div>
					<div class="panel-body">
						<form method="post" enctype="multipart/form-data" action="module/tbdiscount/proses_edit.php">
							<input type="hidden" name="username" value="<?= $user ?>">
							<input type="hidden" name="id" value="<?= $de["id"] ?>">
							<table style=font-size:13px; class="table table-striped table table-bordered">
								<tr>
	...
	?>
		<font face="calibri">
			<div class="panel panel-info">
				<div class="panel-heading">
					<font size="4">TABEL DISCOUNT</font>
				</div>
				<div class="panel-body">
					<form method='post'>
						<div class="row">
							<div class="col-md-12 bg">
								<a class="btn btn-danger btn-sm" href="?m=tbdiscount&tipe=tambah">Tambah data</a>
								<a class="btn btn-success btn-sm" href="?m=tbdiscount&tipe=import">Import data</a>
								<a class="btn btn-warning btn-sm" href="?m=tbdiscount&tipe=export">Export data</a>
							</div>
						</div>
					</form>
					</br>
	...
	<table class="table table-bordered table-striped table-hover" cellspacing="0" width="100%">-->
					<div class="box-body table-responsive">
						<table id="example1" class="table table-bordered table-striped">
							...
							<?php
							$tampil = mysqli_query($connect, "SELECT * FROM tbdiscount order by kode");
							$no = 1;
							while ($k = mysqli_fetch_assoc($tampil)) {
								echo "<tr>
					<td align='center'>$no</td>
					<td><u><a href='?m=tbdiscount&tipe=detail&id=$k[id]'><font color='blue'>$k[kode]</font></a></u></td>
					<td>$k[nama]</td>
					<td>$k[user]</td>
					<td align='center' width='120px'>
						<a class='btn btn-info btn-sm' href='?m=tbdiscount&tipe=edit&id=$k[id]'>Edit</a>";
								cekakses($connect, $user, 'Tabel Discount');
								$lakses = $_SESSION['akseshapus'];
								if ($lakses == 1) {
									//echo " <a class='btn btn-danger' href='module/tbdiscount/proses_hapus.php?id=$k[id]&kode=$k[kode]'
				...
				function alert_hapus($id) {
					swal({
						...
						.then((willDelete) => {
							if (willDelete) {
								//alert($kode);
								$href = "module/tbdiscount/proses_hapus.php?id=";

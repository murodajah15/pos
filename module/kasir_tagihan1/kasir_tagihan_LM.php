    ...
    <?php
		// Cek apakah terdapat data page pada URL
			...
			if (empty($_GET['kata'])){
				...
				$cari = $_GET['kata'];
				$tampil = mysqli_query($connect,"SELECT * FROM kasir_tagihan WHERE (nokwitansi like '%$cari%' or nmsupplier like '%$cari%' or nojual like '%$cari%') order by nokwitansi desc LIMIT ".$limit_start.",".$limit);
			}
			if ($page==1) {
				$posisi=0;
				$_GET['halaman']=1;
        ...
        cekakses($connect,$user,'Pengeluaran Uang');
				$lakses = $_SESSION['aksesedit'];
				if ($lakses == 1) {
	      	if ($k['proses']=='Y') {
		...
		<?php
			if (empty($_GET['kata'])){
				...
				$cari = $_GET['kata'];
				$query = mysqli_query($connect,"select count(*) as jumrec from kasir_tagihan where (nokwitansi like '%$cari%' or nmsupplier like '%$cari%')");
			}
		...
		<ul class="pagination">
			...
			if (empty($_GET['kata'])){
				...
				$cari = $_GET['kata'];
				$tampil = mysqli_query($connect,"SELECT * FROM kasir_tagihan WHERE (nokwitansi like '%$cari%' or nmsupplier like '%$cari%' or nojual like '%$cari%') order by nokwitansi desc ");
			}

<?php
    ...
    $tgl = date('m-d-Y');
		...
		$month = date('M');
		$nmonth = date('m');
		$new_code = 'KWT'.$year.$nmonth.sprintf("%05s", $sort_num);
		return $new_code;
	}
    ...
    if(isset($_GET['tipe'])){
        if($_GET['tipe']=='tambah'){
			...
			$year = date('Y');
			$month = date('M');
			//value="<?php echo autoNumber($connect,'id','kasir_tagihan');
			if ($lakses == 1) {?>
		    <font face='calibri'>
        ...
        <div class='panel-body'>
		      ...
		      <table style=font-size:13px; class='table table-striped table table-bordered' width='600px'>
						...
						<input type='text' class='form-control' id='nokwitansi' name='nokwitansi' placeholder='No. Order *' style='text-transform:uppercase;text-align:left' value="<?php echo autoNumber($connect,'id','kasir_tagihan');?>" readonly></td></tr>
			...
			if ($lakses == 1) {	
				...
				<font face='calibri'>
			        ...
			        <table style=font-size:13px; class='table table-striped table table-bordered' width='600px'>
							...
							<tr><td>Nilai Bayar</td> <td align='right'> <input type="number" class='form-control' id='bayar' name='bayar' size='50' value="<?= $bayar ?>" style='text-align:right' onkeyup="hit_subtotal()" onblur="hit_subtotal()" required></td></tr>
							<tr><td>Uang diterima </td> <td> <input type="number" class='form-control' id='uang' name='uang' size='65' value="<?= $uang ?>" style='text-align:right' onkeyup="hit_subtotal()" onblur="hit_subtotal()" required></td>
  ...
  <div class="panel panel-info">
   	...
   	<form method='post'>
		<div class="row">
			<div class="col-md-12 bg">
				...
				<!--<a class="btn btn-success" href="?m=kasir_tagihan&tipe=import">Import data</a>
				<a class="btn btn-warning" href="?m=kasir_tagihan&tipe=export">Export data</a>-->
				</div>
			</div>
		</form>
    </br>
	...
	// Cek apakah terdapat data page pada URL
        ...
        <td align='center'>$no</td>
				...
				<td width='110'>$k[nmcustomer]</td>
				<td width='110'>$k[bayar]</td>

<body>
	...
	<?php
		...
		if(!empty($_POST['kode'])){
			...
			//Cek Double
			$query = $connect->prepare("select * from tbsatuan where kode=? and id<>?");
			...
			$query = $connect->prepare("update tbsatuan set kode=?,nama=?,user=? where id=?");
			...
			if($query->execute() and mysqli_affected_rows($connect)>0) {
				...
				<script>
					...
					"success"}).then(function(){window.history.go(-2); //window.location.href='../../dashboard.php?m=tbsatuan';
					   }
					);
				</script>
				<?php
				...
				<script>
					...
					"error"}).then(function(){window.history.go(-2); //window.location.href='../../dashboard.php?m=tbsatuan';
					   }
					);
				</script>
				<?php

<body>
	...
	<?php
		...
		//proses simpan
			...
			//Cek Double
			$cek = mysqli_num_rows(mysqli_query($connect,"select * from tbprovinsi where kode='$_POST[kode]' and id<>'$_POST[id]'"));
			if ($cek > 0){
				echo "<script>alert('Kode tersebut sudah digunakan');history.go(-1) </script>";
				exit();
			}
			// $user = "Update-".$_POST['username']."-".date('d-m-Y H:i:s');
			// mysqli_query($connect,"update tbprovinsi set kode='$_POST[kode]',nama='$_POST[nama]',aktif='$_POST[aktif]',user='$user' where id='$_POST[id]'");
			// header("location:../../dashboard.php?m=tbprovinsi");
				...
				$query = $connect->prepare("update tbprovinsi set kode=?,nama=?,aktif=?,user=? where id=?");

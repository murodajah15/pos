<body>
	...
	if (!empty($_POST['kode'])) {
		...
		//Cek Double
		$query = $connect->prepare("select * from tbbarang where kode=? and id<>?");
	...
	?>
		...
		$query = $connect->prepare("update tbbarang set kode=?,nama=?,merek=?,harga_jual=?,harga_beli=?,hpp=?,stock=?,stock_min=?,stock_mak=?,kdjnbrg=?,kdsatuan=?,kdnegara=?,kdmove=?,kddiscount=?,user=?,lokasi=? where id=?");
		$query->bind_param('ssssssssssssssssi', $kode, $nama, $merek, $harga_jual, $harga_beli, $hpp, $stock, $stock_min, $stock_mak, $kdjnbrg, $kdsatuan, $kdnegara, $kdmove, $kddiscount, $user_input, $lokasi, $_POST['id']);
		...
		?>
			<script>
				swal({
					...
					window.history.go(-2); //window.location.href='../../dashboard.php?m=tbbarang';
				});
			</script>
		<?php
		} else {

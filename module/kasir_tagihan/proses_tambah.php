<body>
	...
	<?php
		if(!empty($_POST['nokwitansi'])){
			...
			//  $query->bind_param('ssssssssssssssssssssss',$nokwitansi,$tglkwitansi,$jnskwitansi,$nojual,$kdcustomer,$piutang,$bayar,$uang,$kembali,$carabayar,$kdbank,$nmbank,$kdjnskartu,$nmjnskartu,$norek,$nocekgiro,$tglterimacekgiro,$tgljttempocekgiro,$keterangan,$user_input,$nmcustomer,$username);	

			$query = $connect->prepare("INSERT INTO kasir_tagihan (nokwitansi,tglkwitansi,jnskwitansi,carabayar,kdbank,nmbank,kdjnskartu,nmjnskartu,norek,nocekgiro,tglterimacekgiro,tgljttempocekgiro,keterangan,user,nmcustomer,user_input) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			 $query->bind_param('ssssssssssssssss',$nokwitansi,$tglkwitansi,$jnskwitansi,$carabayar,$kdbank,$nmbank,$kdjnskartu,$nmjnskartu,$norek,$nocekgiro,$tglterimacekgiro,$tgljttempocekgiro,$keterangan,$user_input,$nmcustomer,$username);				
			
			// $query = $connect->prepare("INSERT INTO kasir_tagihan (nokwitansi,tglkwitansi,jnskwitansi,carabayar,kdbank,nmbank) values (?,?,?,?,?,?)");
			//  $query->bind_param('sssssssss',$nokwitansi,$tglkwitansi,$jnskwitansi,$carabayar,$kdbank,$nmbank);				

			// $query = $connect->prepare("INSERT INTO kasir_tagihan (nokwitansi,tglkwitansi,jnskwitansi,nmcustomer,user_input) values (?,?,?,?,?)");
			//  $query->bind_param('sssss',$nokwitansi,$tglkwitansi,$jnskwitansi,$nmcustomer,$username);				

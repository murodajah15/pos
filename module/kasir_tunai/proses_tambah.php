<body>
	...
	<?php
		if(!empty($_POST['nokwitansi'])){
			...
			$nokwitansi = autoNumberKW($connect,'id','kasir_tunai'); //strip_tags($_POST['nokwitansi']);
			//Cek Double
			$cek = mysqli_num_rows(mysqli_query($connect,"select nokwitansi from kasir_tunai where nokwitansi='$nokwitansi'"));
			if ($cek > 0){
			...
			$query = $connect->prepare("INSERT INTO kasir_tunai (nokwitansi,tglkwitansi,jnskwitansi,nojual,kdcustomer,piutang,bayar,uang,kembali,carabayar,kdbank,nmbank,kdjnskartu,nmjnskartu,norek,nocekgiro,tglterimacekgiro,tgljttempocekgiro,keterangan,user,nmcustomer,user_input) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			// $query = $connect->prepare("INSERT INTO kasir_tunai (nokwitansi,tglkwitansi,jnskwitansi,nojual,kdcustomer,piutang) values (?,?,?,?,?,?)");
			...
			if($query->execute() and mysqli_affected_rows($connect)>0){
				...
				// </script>";							
				$aktif = 'Y';
				$query = $connect->prepare("update saplikasi set nokwtunai=? where aktif=?");
				...
				<script>
					...
					"success"}).then(function(){window.location.href='../../dashboard.php?m=kasir_tunai';
					   }
					);
				</script>
				<?php
				...
				<script>
					...
					"error"}).then(function(){window.location.href='../../dashboard.php?m=kasir_tunai';
					   }
					);
				</script>
				<?php
			...
			header("location:../../dashboard.php?m=kasir_tunai");
		}
	?>
</body>
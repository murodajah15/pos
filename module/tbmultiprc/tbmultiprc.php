			...
			<font face='calibri'>
				<div class="panel panel-success">
					<div class="panel-heading">
						<font size="4">IMPORT DATA TABEL MULTI PRICE</font>
					</div>
		...
		if ($lakses == 1) { ?>
			<font face='calibri'>
				<div class="panel panel-default">
					<div class="panel-heading">
						<font size="4">EXPORT DATA TABEL MULTI PRICE</font>
					</div>
					<div class="panel-body">
...
if (isset($_GET['tipe'])) {
	if ($_GET['tipe'] == 'edit_detail') {
		...
		if ($lakses == 1) {
			...
			$sql = mysqli_query($connect, "select * from tbmultiprc where id='$_GET[id]'");
		...
		?>
			<font face="calibri">
				<div class="panel panel-warning">
					<div class="panel-heading">
						<font size="4">EDIT DATA TABEL MULTI PRICE</font>
					</div>
					<div class="panel-body">
						<form method="post" enctype="multipart/form-data" action="module/tbmultiprc/proses_edit_detail.php">
							...
							<div class='col-md-6'>
								<table style=font-size:13px; class="table table-striped table table-bordered">
									<tr>
										<td>Kode Barang</td>
										<td> <input type="text" class="form-control" name="kdbarang" value="<?= $kdbarang ?>" readonly></td>
									</tr>
									<tr>
										<td>Nama Barang</td>
										<td> <input type="text" class="form-control" name="nmbarang" id="nmbarang" value="<?= $nmbarang ?>" readonly></td>
									</tr>
									<tr>
										<td>Harga</td>
									...
									<tr>
										<td>Discount (%)</td>
										<td> <input type='number' step='any' onfocus="this.select();" max='100' class='form-control' name='discount' value="<?= $discount ?>" autofocus="autofocus"></td>
									</tr>
								</table>
							</div>
							<div class='col-md-12'>
...
if (isset($_GET['tipe'])) {
	if ($_GET['tipe'] == 'detail_data') {
		...
		if ($lakses == 1) {
			...
			// $result = $query->get_result();
			// $de = $result->fetch_assoc();
			$sql = mysqli_query($connect, "select * from tbcustomer where kode='$_GET[kdcustomer]'");
			$de = mysqli_fetch_assoc($sql);
		...
		?>
			<font face="calibri">
				<div class="panel panel-warning">
					<div class="panel-heading">
						<font size="4">DETAIL DATA TABEL MULTI PRICE</font>
					</div>
					<div class="panel-body">
						<form method="post" enctype="multipart/form-data" action='module/tbmultiprc/proses_tambah_detail.php'>
							...
							<div class='col-md-12'>
								<table style=font-size:13px; class="table table-striped table table-bordered">
									<tr>
										<td>Kode</td>
										<td> <input type='text' class='form-control' name='kdcustomer' value='<?= $kdcustomer ?>' size='10' required readonly></td>
										<td>nama</td>
										<td> <input type='text' class='form-control' name='nmcustomer' value='<?= $nmcustomer ?>' size='80' required readonly></td>
									</tr>
								</table>
							</div>
							...
							<div class='col-md-12'>
								...
								if ($lakses == 1) { ?>
									<div class='col-md-3'>
										<input button type='Button' class='btn btn-danger  btn-sm' value='Salin dari Tabel Barang' onClick='salin_tbbarang("<?= $kdcustomer ?>")' />
									</div>
									<div class='col-md-8'>
										<table style=font-size:13px; class="table table-striped table table-bordered">
											<td>
												<div class='input-group'> <input type='text' class='form-control' id='kdcustomer' name='kdcustomer_salin' size='20%' autocomplete='off' style='text-transform:uppercase' required readonly>
													<span class='input-group-btn'>
														<button type='button' id='src' class='btn btn-default btn-sm' onclick='cari_data_customer()'>Cari</button>
													</span><input type="text" class='form-control' id='nmcustomer' name='nmcustomer' size='50' readonly required>
											</td>
											<td>
												<input button type='Button' class='btn btn-primary btn-sm' value='Salin dari Customer' onClick='salin_tbbarang_customer("<?= $kdcustomer ?>")' />
											</td>
										</table>
									</div>
								<?php
									...
									<input button type='Button' class='btn btn-primary btn-sm' value='Salin dari Tabel Barang' onClick='salin_tbbarang("<?= $kdcustomer ?>")' disabled />
								<?php
								}
								?>
								<table style=font-size:13px; class="table table-striped table table-bordered">
									...
									<td>
										<div class='input-group'> <input type='text' class='form-control' id='kdbarang' name='kdbarang' size='50' autocomplete='off' style='text-transform:uppercase' required>
											<span class='input-group-btn'>
												<button type='button' id='src' class='btn btn-primary btn-sm' onclick='cari_data_tbbarang()'>Cari</button>
											</span>
									</td>
									</td>
									<td><input type='text' class='form-control' style='width: 15em' id='nmbarang' name='nmbarang' readonly></td>
									</td>
									<td><input type="text" class='form-control' id='harga' name='harga' style='width: 8em'></td>
									</td>
									<td><input type="text" max='100' class='form-control' id='discount' name='discount' style='width: 6em' onkeyup="validAngka(this)"></td>
				        ...
				        <input button type='Button' class='btn btn-danger btn-sm' value='+' onClick='tambah_detail($k[id])'/>-->
							...
							<div class='col-md-12'>
								...
								<table id="example1" class="table table-bordered table-striped">
									...
									$tampil = mysqli_query($connect, "select * from tbmultiprc where kdcustomer='$kdcustomer'");
									// $tampil = mysqli_query($connect, "select coalesce(kdbarang,nmbarang,harga,discount) from tbmultiprc where kdcustomer='$kdcustomer'");
			        ...
			        <td align='center'>$no</td>
							...
							<td align='center' width='100px'>";
										...
										if ($aksesedit == 1) {
											echo "<a class='btn btn-primary btn-sm' href='?m=tbmultiprc&tipe=edit_detail&id=$k[id]'>Edit</a> ";
										} else {
											echo "<a class='btn btn-primary btn-sm' href='?m=tbmultiprc&tipe=edit_detail&id=$k[id]' disabled>Edit</a> ";
										}
										if ($akseshapus == 1) {
											echo "<input button type='Button' class='btn btn-danger btn-sm' value='Hapus' onClick='alert_hapus_detail($k[id])'/>";
										} else {
											echo "<input button type='Button' class='btn btn-danger btn-sm' value='Hapus' onClick='alert_hapus_detail($k[id])' disabled/>";
										}
										$no++;
									}
									?>
							...
							<div class='col-md-12'>
								<input button type='Button' class='btn btn-danger btn-sm' value='Close' onClick="window.location.href='?m=tbmultiprc'" />
							</div>
					</div>
				</div>
				</form>
...
if (isset($_GET['tipe'])) {
		...
		if ($lakses == 1) { ?>
			<font face='calibri'>
				<div class='panel panel-danger'>
					<div class='panel-heading'>
						<font size="4">TAMBAH DATA TABEL MULTI PRICE</font>
					</div>
					<div class='panel-body'>
	...
	?>
		...
		<font face="calibri">
			<div class="panel panel-info">
				<div class="panel-heading">
					<font size="4">TABEL MULTI PRICE</font>
				</div>
				<div class="panel-body">
					<form method='get'>
						<div class="row">
					...
					<input type="hidden" name="m" value="tbmultiprc">
					<input type='text' id='kata' name='kata' value="<?= $kata ?>" size='50px' class='form-control' placeholder='KODE,NAMA,ALAMAT' onkeyup='searchTable()'>
				</div>
				...
				<span class='glyphicon glyphicon-search'></span> Cari</button> -->
					...
					<div class="box-body table-responsive">
						<table id="tbmultiprice" class="table table-bordered table-hover">
							<thead>
								<tr>
									<!-- <th width='40'>No</th> -->
									<th width='40'>Kode</th>
	...
	<script>
		...
		function salin_tbbarang_customer($id) {
			$("#imgLoad").show("");
			$kdcustomersalin = document.getElementById('kdcustomer').value;
			swal({
					title: "Yakin akan salin tabel barang dari customer " + $kdcustomersalin + " ?",
					text: "",
					icon: "warning",
					buttons: true,
					dangerMode: true,
				...
				.then((willDelete) => {
					if (willDelete) {
						$href = "module/tbmultiprc/salin_tbbarang_customer.php?id=";
						window.location.href = $href + $id + '&kdcustomersalin=' + $kdcustomersalin;
						// swal("Poof! Your imaginary file has been deleted!", {
						//   icon: "success",
						// });
					} else {

<body>
	...
	<?php
		...
		if ($de['proses']=='Y' or $de['total']==0) {
			$text = "Dokumen ".$noterima." Sudah di proses / total masih 0 (nol)!";
			?>
			<script>
		...
		$rec = mysqli_num_rows(mysqli_query($connect,"select * from terimad where noterima='$noterima'"));
		if ($rec==0) {
		...
		$query = $connect->prepare("update terimah set proses=?,subtotal=?,total=?,user_proses=? where id=?");
		$query->bind_param('ssssi',$proses,$nsubtotal,$ntotal,$user_proses,$_GET['id']);
		if($query->execute() and mysqli_affected_rows($connect)>0){
			mysqli_query($connect,"update terimad set proses='$proses',tglterima='$tglterima',jamterima='$jamterima' where noterima='$noterima'");
			$tampil = mysqli_query($connect,"select * from terimad where noterima='$noterima'");
     		while($k=mysqli_fetch_assoc($tampil)){
				...
				$hpp = $de['hpp'];
				mysqli_query($connect,"update terimad set hpp='$hpp' where noterima='$noterima' and kdbarang='$kdbarang'");

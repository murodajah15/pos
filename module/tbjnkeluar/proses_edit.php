<body>
	...
	<?php
		...
		if(!empty($_POST['kode'])){
			...
			//Cek Double
			$query = $connect->prepare("select * from tbjnkeluar where kode=? and id<>?");
			...
			if ($query->num_rows >= "1") {
				...
				<script>
					...
					"error"}).then(function(){window.location.href='../../dashboard.php?m=tbjnkeluar';
					   }
					);
				</script>
				<?php
			...
			$query = $connect->prepare("update tbjnkeluar set kode=?,nama=?,user=? where id=?");
			...
			if($query->execute() and mysqli_affected_rows($connect)>0) {
				...
				<script>
					...
					"success"}).then(function(){window.history.go(-2); //window.location.href='../../dashboard.php?m=tbjnkeluar';
					   }
					);
				</script>
				<?php
				...
				<script>
					...
					"error"}).then(function(){window.history.go(-2); //window.location.href='../../dashboard.php?m=tbjnkeluar';
					   }
					);
				</script>
				<?php

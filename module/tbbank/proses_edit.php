<body>
	...
	<?php
		...
		if(!empty($_POST['kode'])){
			...
			//Cek Double
			$query = $connect->prepare("select * from tbbank where kode=? and id<>?");
			...
			//echo $user_input.'   '.$kode.'    '.$nama.'  '.$_POST['id'];
			$query = $connect->prepare("update tbbank set kode=?,nama=?,user=? where id=?");

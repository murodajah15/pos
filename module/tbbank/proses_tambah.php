<body>
	...
	<?php
		if(!empty($_POST['nama'])){
			...
			$query = $connect->prepare("select * from tbbank where kode=?");
			...
			$query = $connect->prepare("INSERT INTO tbbank (kode,nama,user) values (?,?,?)");
			...
			if($query->execute() and mysqli_affected_rows($connect)>0){
				...
				<script>
					...
					"success"}).then(function(){window.location.href='../../dashboard.php?m=tbbank';
					   }
					);
				</script>
				<?php
				...
				<script>
					...
					"error"}).then(function(){window.location.href='../../dashboard.php?m=tbbank';
					   }
					);
				</script>
				<?php

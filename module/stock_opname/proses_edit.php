<body>
	...
	<?php
		...
		if(!empty($_POST['kode'])){
			...
			$query = $connect->prepare("update tbcustomer set kode=?,kelompok=?,nama=?,alamat=?,kota=?,kodepos=?,telp1=?,telp2=?,agama=?,tgl_lahir=?,alamat_ktr=?,kota_ktr=?,kodepos_ktr=?,telp1_ktr=?,telp2_ktr=?,npwp=?,alamat_npwp=?,nama_npwp=?,alamat_ktp=?,kota_ktp=?,kodepos_ktp=?,tgl_register=?,user=? where id=?");
			$query->bind_param('sssssssssssssssssssssssi',$kode,$kelompok,$nama,$alamat,$kota,$kodepos,$telp1,$telp2,$agama,$tgl_lahir,$alamat_ktr,$kota_ktr,$kodepos_ktr,$telp1_ktr,$telp2_ktr,$npwp,$alamat_npwp,$nama_npwp,$alamat_ktp,$kota_ktp,$kodepos_ktp,$tgl_register,$user_input,$_POST['id']);

		...
		if(!empty($_POST['nama'])){
			...
			if ($query->num_rows >= "1") {
				...
				<script>
					...
					"error"}).then(function(){window.history.go(-1);} //{window.location.href='../../dashboard.php?m=tbcustomer';
					   //}
					);
				</script>
				<?php
			...
			$query = $connect->prepare("INSERT INTO tbcustomer (kode,kelompok,nama,alamat,kota,kodepos,telp1,telp2,agama,tgl_lahir,alamat_ktr,kota_ktr,kodepos_ktr,telp1_ktr,telp2_ktr,npwp,alamat_npwp,nama_npwp,alamat_ktp,kota_ktp,kodepos_ktp,tgl_register,user) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			$query->bind_param('sssssssssssssssssssssss',$kode,$kelompok,$nama,$alamat,$kota,$kodepos,$telp1,$telp2,$agama,$tgl_lahir,$alamat_ktr,$kota_ktr,$kodepos_ktr,$telp1_ktr,$telp2_ktr,$npwp,$alamat_npwp,$nama_npwp,$alamat_ktp,$kota_ktp,$kodepos_ktp,$tgl_register,$user_input);
			...
			if($query->execute() and mysqli_affected_rows($connect)>0){
				...
				<script>
					...
					"success"}).then(function(){window.location.href='../../dashboard.php?m=tbcustomer';
					   }
					);
				</script>
				<?php

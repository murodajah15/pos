<body>
	...
	<?php
		...
		//proses simpan
			...
			//Cek Double
			$cek = mysqli_num_rows(mysqli_query($connect,"select * from belih where nobeli='$_POST[nobeli]' and id<>'$_POST[id]'"));
			if ($cek > 0){
				...
				$ntotal=hit_total($connect,$nobeli,$ppn);
				$total_sementara = $_POST['biaya_lain']+$nsubtotal;
				$ntotal = $total_sementara + ($total_sementara*($ppn/100)) + $materai;
				$query = $connect->prepare("update belih set nobeli=?,tglbeli=?,noinvoice=?,tglinvoice=?,kdsupplier=?,nmsupplier=?,jenis_order=?,tglkirim=?,biaya_lain=?,ket_biaya_lain=?,carabayar=?,ppn=?,materai=?,keterangan=?,user=?,subtotal=?,total=?,tempo=?,tgl_jt_tempo=?,total_sementara=? where id=?");
				$query->bind_param('sssssssssssssssssssss',$nobeli,$tglbeli,$noinvoice,$tglinvoice,$kdsupplier,$nmsupplier,$jenis_order,$tglkirim,$biaya_lain,$ket_biaya_lain,$carabayar,$ppn,$materai,$keterangan,$user_input,$nsubtotal,$ntotal,$tempo,$tgl_jt_tempo,$total_sementara,$_POST['id']);				
	...
	<?php
	//pembuatan fungsi
	function hit_total($connect,$nobeli,$ppn)
	{
		global $nsubtotal;
		include "../../inc/config.php";
		$tampil = mysqli_query($connect,"select sum(subtotal) as nsubtotal from belid where nobeli='$nobeli'");

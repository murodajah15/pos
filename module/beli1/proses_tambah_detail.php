			...
			if(!empty($_POST['nopo'])){
				...
				if ($_POST['kdbarang']=="") {
					...
					<script>
						...
						"error"}).then(function(){window.location.href='../../dashboard.php?m=beli&tipe=detail&nobeli=$nobeli';
						   }
						);
					</script>
					<?php
				...
				if($query->execute() and mysqli_affected_rows($connect)>0){
					...
					<script>
						...
						"success"}).then(function(){window.location.href='../../dashboard.php?m=beli&tipe=detail&nobeli=$nobeli';
						   }
						);
					</script>
					<?php

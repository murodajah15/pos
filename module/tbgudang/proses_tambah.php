<body>
	...
	<?php
		if(!empty($_POST['nama'])){
			...
			$query = $connect->prepare("select * from tbgudang where kode=?");
			...
			if ($query->num_rows >= "1") {
				...
				<script>
					...
					"error"}).then(function(){window.history.go(-1);} //{window.location.href='../../dashboard.php?m=tbgudang';
					   //}
					);
				</script>
				<?php
			...
			$query = $connect->prepare("INSERT INTO tbgudang (kode,nama,user) values (?,?,?)");
			...
			if($query->execute() and mysqli_affected_rows($connect)>0){
				...
				<script>
					...
					"success"}).then(function(){window.location.href='../../dashboard.php?m=tbgudang';
					   }
					);
				</script>
				<?php

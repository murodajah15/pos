<?php
  ...
  if(isset($_GET['tipe'])){
    if($_GET['tipe']=='detail_proses'){
        ...
        <div class='panel-heading'><font size="4">DETAIL PENGELUARAN UANG</font></div>
        <div class='panel-body'>
		      ...
		      <!--<table style=font-size:13px; class='table table-striped table table-bordered' width='600px'>-->
						...
						<input type='text' class='form-control' id='nokwitansi' name='nokwitansi' placeholder='No. PO *' style='text-transform:uppercase' value="<?= $nokwitansi ?>" readonly></td></tr>
						<tr><td>Tgl. (M/D/Y)</td><td><input type='date' class='form-control' id='tglkwitansi' name='tglkwitansi' value="<?= $tglkwitansi ?>" size='50' autocomplete='off' readonly></td></tr>
							<tr><td>Jenis Pengeluaran</td> <td><input type='text' class='form-control' name='kdjnkeluar' id='kdjnkeluar' size='20' value='<?=$kdjnkeluar?>' autocomplete='off' readonly required>
				...
				<div class='col-md-6'>
					<table class="table table-bordered table-striped table-hover">
							<tr><td>Subtotal</td> <td> <input type="number" class='form-control' name='subtotal' id='subtotal' size='50' value='<?= $subtotal ?>' readonly></td></tr>
		 					<tr><td>Materai</td> <td> <input type="number" class='form-control' name='materai' id='materai' size='50' value='<?= $materai ?>' readonly"></td></tr>							
		 					<tr><td>Total</td> <td> <input type="number" class='form-control' name='total' id='total' size='50' value='<?= $total ?>' readonly></td></tr>
							<tr><td>Keterangan</td><td><textarea type='text' class='form-control' id='keterangan' name='keterangan' autocomplete='off' readonly><?= $keterangan ?></textarea></td></tr>
					</table>
				</div>
				<div class='col-md-12'>
					<table class="table table-bordered table-striped table-hover">
  ...
  if(isset($_GET['tipe'])){
    if($_GET['tipe']=='detail'){
		...
		if ($lakses == 1) {?>
			<font face='calibri'>
        <div class='panel panel-danger'>
        <div class='panel-heading'><font size="4">TAMBAH DATA DETAIL KASIR PENGELUARAN UANG</font></div>
        <div class='panel-body'>
				  ...
				  <span class='input-group-btn'></span>
					...
					<div class='col-md-12'>
					    ...
					    <th>Aksi</th>
							...
							</td><td><input type='text' class='form-control' style='width: 15em' id='nmsupplier' name='nmsupplier'></td>
							...
							</td><td><input type="number" class='form-control' id='uang' name='uang' style='width: 8em' onkeyup="hit_subtotal()" onblur="hit_subtotal()"></td>
				...
				<font face='calibri'>
					...
					<form method='post' enctype='multipart/form-data' action='module/kasir_keluar/proses_edit_detail.php'>
						...
						<input type='hidden' name='nokwitansi' value='<?= $nokwitansi ?>'>
 						Nomor Kwitansi
						<input type='text' class='form-control' style='width: 15em' id='nokwitansi' name='nokwitansi' placeholder='No. Kwitansi *' style='text-transform:uppercase' value="<?= $nokwitansi ?>" 
						readonly>
						<br>
  ...
  if(isset($_GET['tipe'])){
    if($_GET['tipe']=='tambah'){
      ...
      if ($lakses == 1) {?>
        <font face='calibri'>
        <div class='panel panel-danger'>
        <div class='panel-heading'><font size="4">TAMBAH DATA KASIR PENGELUARAN UANG</font></div>
        <div class='panel-body'>
		      ...
		      <table style=font-size:13px; class='table table-striped table table-bordered' width='600px'>
					<tr><td>Nomor Kwitansi</td><td>
					<input type='text' class='form-control' id='nokwitansi' name='nokwitansi' id='nokwitansi' placeholder='No. Kwitansi *' style='text-transform:uppercase' value="<?php echo autoNumberKK($connect,'id','kasir_keluarh');?>" readonly></td></tr>
					...
					<tr><td>Keterangan</td><td><textarea type='text' rows='3' class='form-control' id='kerangan' name='keterangan' autocomplete='off'></textarea></td></tr>
					</table>
				</div>
				<div class='col-md-6'>
		      <table style=font-size:13px; class='table table-striped table table-bordered' width='600px'>
			...
			if ($lakses == 1) {	
				...
				<font face='calibri'>
					...
					<div class="panel-heading"><font size="4">EDIT DATA KASIR PENGELUARAN UANG</font></div>
					<div class="panel-body">
			        ...
			        <table style=font-size:13px; class='table table-striped table table-bordered' width='600px'>
							<tr><td>Nomor Kwitansi</td><td>
							<input type='text' class='form-control' id='nokwitansi' name='nokwitansi' placeholder='No. Jual *' style='text-transform:uppercase' value="<?= $nokwitansi ?>" readonly></td></tr>
							<tr><td>Tgl. Penjualan (M/D/Y)</td><td><input type='date' class='form-control' id='tglkwitansi' name='tglkwitansi' value="<?php echo $tglkwitansi ?>" size='50' autocomplete='off' required readonly></td></tr>
  ...
  <div class="panel panel-info">
    <div class="panel-heading"><font size="4">KASIR PENGELUARAN UANG</font></div>
    <div class="panel-body">
	 	<form method='get'>
			<div class="row">
				<?php
				...
				<div class="col-md-4 bg">
					<input type="hidden" name="m" value="kasir_keluar">
					<input type='text' id='kata' name='kata' value="<?= $kata ?>" size='50px' class='form-control' placeholder='NO.KWITANSI'  onkeyup='searchTable()'>
				</div>
    ...
    <th width='300'>Aksi</th>
		...
		<?php
			...
			if (empty($_GET['kata'])){
				$tampil = mysqli_query($connect,"SELECT * FROM kasir_keluarh order by nokwitansi desc LIMIT ".$limit_start.",".$limit);
				}
			else{
				$cari = $_GET['kata'];
				$tampil = mysqli_query($connect,"SELECT * FROM kasir_keluarh WHERE (nokwitansi like '%$cari%') order by nokwitansi desc LIMIT ".$limit_start.",".$limit);
			}
			if ($page==1) {
				$posisi=0;
				$_GET['halaman']=1;
		...
		<?php
			if (empty($_GET['kata'])){
				$query = mysqli_query($connect,"select count(*) as jumrec from kasir_keluarh");
			}else{
				$cari = $_GET['kata'];
				$query = mysqli_query($connect,"select count(*) as jumrec from kasir_keluarh where (nokwitansi like '%$cari%')");
			}
		...
		<ul class="pagination">
			...
			?>
			  <li><a href="dashboard.php?m=kasir_keluar&kata=<?= $kata ?>&page=1">First</a></li>
			  <li><a href="dashboard.php?m=kasir_keluar&kata=<?= $kata ?>&page=<?php echo $link_prev; ?>">&laquo;</a></li>
			<?php
			}
			?>
			<!-- LINK NUMBER -->
			...
			if (empty($_GET['kata'])){
				...
				$cari = $_GET['kata'];
				$tampil = mysqli_query($connect,"SELECT * FROM kasir_keluarh WHERE (nokwitansi like '%$cari%') order by nokwitansi desc ");
			}
			...
			?>
			  <li<?php echo $link_active; ?>><a href="dashboard.php?m=kasir_keluar&kata=<?= $kata ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a></li>
			<?php
			}
			?>
			...
			if($page == $jumlah_page){ // Jika page terakhir
			  ...
			  $link_next = ($page < $jumlah_page)? $page + 1 : $jumlah_page;
				?>
			  <li><a href="dashboard.php?m=kasir_keluar&kata=<?= $kata ?>&page=<?php echo $link_next; ?>">&raquo;</a></li>
			  <li><a href="dashboard.php?m=kasir_keluar&kata=<?= $kata ?>&page=<?php echo $jumlah_page; ?>">Last</a></li>
			<?php
			}
			?>
		</ul>

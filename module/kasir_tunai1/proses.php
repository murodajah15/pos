	...
	<?php
		...
		if ($bayar > $kurangbayar) {
			...
			<script>
				...
				"error"}).then(function(){window.history.back(); //window.location.href='../../dashboard.php?m=kasir_tunai';
				   }
				);
			</script>
			<?php
		...
		if($query->execute() and mysqli_affected_rows($connect)>0){
			...
			<script>
				...
				"success"}).then(function(){window.history.back(); //window.location.href='../../dashboard.php?m=kasir_tunai';
				   }
				);
			</script>
			<?php

<body>
	...
	<?php
		...
		//proses simpan
			...
			//Cek Double
			$cek = mysqli_num_rows(mysqli_query($connect,"select * from kasir_tunai where nokwitansi='$_POST[nokwitansi]' and id<>'$_POST[id]'"));
			if ($cek > 0){
				...
				$query = $connect->prepare("update kasir_tunai set nokwitansi=?,tglkwitansi=?,jnskwitansi=?,nojual=?,kdcustomer=?,piutang=?,bayar=?,uang=?,kembali=?,carabayar=?,kdbank=?,nmbank=?,kdjnskartu=?,nmjnskartu=?,norek=?,nocekgiro=?,tglterimacekgiro=?,tgljttempocekgiro=?,keterangan=?,user=?,nmcustomer=?,user_input=? where id=?");
				$query->bind_param('ssssssssssssssssssssssi',$nokwitansi,$tglkwitansi,$jnskwitansi,$nojual,$kdcustomer,$piutang,$bayar,$uang,$kembali,$carabayar,$kdbank,$nmbank,$kdjnskartu,$nmjnskartu,$norek,$nocekgiro,$tglterimacekgiro,$tgljttempocekgiro,$keterangan,$user_input,$nmcustomer,$username,$_POST['id']);

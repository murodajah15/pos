<body>
	...
	<?php
		if(!empty($_POST['noso'])){
			...
			if (!empty($kdbarang)) {
				$cek = mysqli_num_rows(mysqli_query($connect,"select * from sod where kdbarang='$kdbarang' and noso='$_POST[noso]'"));
				if ($cek > 0){
					echo "<script>alert('Double barang, data tidak bisa disimpan !');history.go(-1) </script>";
					exit();
				}
				$cek = mysqli_num_rows(mysqli_query($connect,"select kode from tbbarang where kode='$kdbarang'"));
				if ($cek == 0){
				...
				$query = $connect->prepare("INSERT INTO sod (noso,kdbarang,nmbarang,kdsatuan,qty,harga,discount,subtotal,user) values (?,?,?,?,?,?,?,?,?)");
				$query->bind_param('ssssiiiis',$noso,$kdbarang,$nmbarang,$kdsatuan,$qty,$harga,$discount,$subtotal,$user_input);
			...
			header("location:../../dashboard.php?m=so&tipe=detail&noso=$noso&kdcustomer=$kdcustomer");
		}
	?>

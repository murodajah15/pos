		...
		if ($_POST['password']=="") {
			...
			if($query->execute() and mysqli_affected_rows($connect)>0) {
				...
				<script>
					...
					"success"}).then(function(){window.location.href='../../dashboard.php?m=user';
					   }
					);
				</script>
				<?php
				...
				<script>
					...
					"error"}).then(function(){window.location.href='../../dashboard.php?m=user';
					   }
					);
				</script>
				<?php
			...
			if ($cek > 0){
				...
				?>
				<script>
					swal({title: "User Name sudah ada ", text: "data tidak bisa disimpan!", icon: 
					"error"}).then(function(){window.location.href='../../dashboard.php?m=user';
					   }
					);
				</script>
			...
			if($query->execute() and mysqli_affected_rows($connect)>0){
				...
				?>
				<script>
					swal({title: "Gagal simpan1 data!", text: "", icon: 

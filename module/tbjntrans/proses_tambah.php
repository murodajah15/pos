<body>
	...
	<?php
		if(!empty($_POST['nama'])){
			...
			$query = $connect->prepare("select * from tbjntrans where kode=?");
			...
			if ($query->num_rows >= "1") {
				...
				<script>
					...
					"error"}).then(function(){window.history.go(-1);} //{window.location.href='../../dashboard.php?m=tbjntrans';
					   //}
					);
				</script>
				<?php
			...
			$query = $connect->prepare("INSERT INTO tbjntrans (kode,nama,keterangan,user) values (?,?,?,?)");
			$query->bind_param('ssss',$kode,$nama,$keterangan,$user_input);
			if($query->execute() and mysqli_affected_rows($connect)>0){
				...
				<script>
					...
					"success"}).then(function(){window.location.href='../../dashboard.php?m=tbjntrans';
					   }
					);
				</script>
				<?php
